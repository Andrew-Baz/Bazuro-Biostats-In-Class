...
```{r}
carnivores <- critters |> filter(Order == "Carnivora") |> filter(Class == "Mammalia")
```

create a histogram of maximum longevity for carnivores.

```{r}
ggplot(data = carnivores, aes(x = max_longevity_years)) +
  geom_histogram()
```
...
```{r}
ggplot(data = carnivores, aes(x = max_longevity_years, y = adult_wt_g)) +
  geom_point()
```
...
```{r}
carnivores |> filter(max_longevity_years == max(max_longevity_years))
critters |> filter(max_longevity_years == max(max_longevity_years))
##longest lived carnivore is the Baikal seal. Longest lived overall species is the Bowhead whale.
```

Use code to determine the two species between 2.5e+05 and 3e+05 in adult body mass.

```{r}
carnivores |> filter(adult_wt_g > 250000 & adult_wt_g < 300000)
```
...
```{r}
ggplot(data = carnivores, aes(x = family, y = max_longevity_years)) +
  geom_boxplot() +
  labs()
```
