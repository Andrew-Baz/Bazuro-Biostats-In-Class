...
```{r}
bats$Bat_ID <- as.factor(bats$Bat_ID)
...
```{r}
ggplot(data = bats, aes(x = Arousals, y = mass_loss_g)) +
  geom_point()
```
...
Roughly speaking, the mass lost during hibernation ranges between **.2-1.9**

Roughly speaking, the number of arousals varies between **4-16**

Slope is rise/run, which in this case is about **1.7** for rise and **12** for run, so we get **0.1416667** as our guesstimate of the slope.

Eyeballing the plot, we would also guess that the y-intercept is somewhere around **0**
...
##The data follows a normal distribution, and has equal variance.
```
